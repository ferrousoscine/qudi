...
class LaserQuantumLaser(Base, SimpleLaserInterface):
    ...
    def set_control_mode(self, mode):
        ...
        if self.psu == PSUTypes.FPU:
            ...
            if mode == ControlMode.POWER:
                ...
                self.log.debug(f"Set POWER control mode {reply1}, {reply2}.")
            else:
                reply1 = self.inst.query("PFB=ON")
                reply2 = self.inst.query("CONTROL=CURRENT")
                self.log.debug(f"Set CURRENT control mode {reply1}, {reply2}.")
    ...
    def get_power(self):
        ...
        try:
            ...
            self.log.exception(f"Answer was {answer}.")
            return -1
    ...
    def set_power(self, power):
        ...
        """
        if self.psu == PSUTypes.FPU:
            self.inst.query(f"POWER={power:f}")
        else:
            self.inst.query(f"POWER={power * 1000:f}")
    ...
    def set_current(self, current_percent):
        ...
        """
        self.inst.query(f"CURRENT={current_percent}")
