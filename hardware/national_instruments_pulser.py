...
class NationalInstrumentsPulser(Base, PulserInterface):
    ...
    def on_activate(self):
        ...
        if "pulsed_file_dir" in config.keys():
            ...
            self.log.warning(
                ...
                f"as directory for the pulsed files!\nThe default home directory\n{self.pulsed_file_dir}\n"
                "will be taken instead."
            )
    ...
    def init_constraints(self):
        ...
        self.log.debug(f"Analog min freq: {ao_min_freq.value}")
        daq.DAQmxGetDevAOMaxRate(device, daq.byref(ao_max_freq))
        self.log.debug(f"Analog max freq: {ao_max_freq.value}")
        daq.DAQmxGetDevAOSampClkSupported(device, daq.byref(ao_clock_support))
        self.log.debug(f"Analog supports clock: {ao_clock_support.value}")
        ...
        for n, ch in enumerate(analog_channels):
            ch_map[f"a_ch{n + 1:d}"] = ch

        for n, ch in enumerate(digital_channels):
            ch_map[f"d_ch{n + 1:d}"] = ch
        ...
        constraints["sampled_file_length"] = {"min": 2, "max": 1e12, "step": 0, "unit": "Samples"}
    ...
    def load_asset(self, asset_name, load_dict=None):
        ...
        if asset_name not in saved_assets:
            self.log.error(
                f'No asset with name "{asset_name}" found for NI pulser.\n"load_asset" call ignored.'
            )
            return -1
