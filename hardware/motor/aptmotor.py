...
class APTMotor:
    ...
    error_code = {
        10000: "An unknown Server error has occurred. ",
        ...
        "corrupted.",
        10051: "An error has occurred with the ethernet connections or the windows sockets. ",
        10052: "An error has occurred whilst accessing the registry. ",
        10053: "An internal memory allocation error or de-allocation error has occurred.",
        10054: "An error has occurred with the COM system. Restart the program.",
        10055: "An error has occurred with the USB communications.",
        10100: "A serial number has been specified that is unknown to the server.",
        ...
        10104: "A message has been received with an unknown identifier.",
        ...
        10150: "The GetStageAxisInfo method has been called when no stage has been assigned. ",
        10151: "An internal error has occurred when using an encoded stage.",
        10152: "An internal error has occurred when using an encoded stage. ",
    ...
    status_code = {
        1: "0x00000001, 1, forward hardware limit switch is active. "
        ...
        4: "0x00000008, 4, CCW software limit switch (0 - no "
        "contact, 1 - contact). Not applicable to Part Number "
        "ODC001 and TDC001 controllers",
        ...
        "ODC001 and TDC001 controllers.",
        10: "0x00000200, 10, in motion, homing, Motor homing (1 - homing, 0 - not homing).",
        11: "0x00000400, 11, homed (homing has been completed)(1 - homed, 0 - not homed).",
        12: "0x00000800, 12, For Future Use.",
        ...
        "window).",
        15: "0x00004000, 15, motion error (excessive position "
        ...
        "no error).",
        17: "0x00010000, 17, Interlock link missing in motor connector (1 – missing, 0 – present).",
        18: "0x00020000, 18, Position module over temperature "
        "warning (1 – over temp, 0 – temp OK).",
        19: "0x00040000, 19, Position module bus voltage fault (1 – fault exists, 0 – OK).",
        20: "0x00080000, 20, Axis commutation error (1 – error, 0 – OK).",
        21: "0x00100000, 21, Digital input 1 state (1 - logic high, 0 - logic low).",
        22: "0x00200000, 22, Digital input 2 state (1 - logic high, 0 - logic low).",
        23: "0x00400000, 23, Digital input 3 state (1 - logic high, 0 - logic low).",
        24: "0x00800000, 24, Digital input 4 state (1 - logic high, 0 - logic low).",
        25: "0x01000000, 25, BBD10x Controllers: Axis phase "
        "current limit (1 – current limit exceeded, "
        "0 – below limit). Other Controllers: Digital input 5 "
        "state (1 - logic high, 0 - logic low).",
        26: "0x02000000, 26, Digital input 6 state (1 - logic high, 0 - logic low).",
        27: "0x04000000, 27, Unspecified, for Future Use.",
        28: "0x08000000, 28, Unspecified, for Future Use.",
        29: "0x10000000, 29, Unspecified, for Future Use.",
        30: "0x20000000, 30, Active (1 – indicates unit is active, 0 – not active).",
        31: "0x40000000, 31, Unspecified, for Future Use.",
    ...
    def set_stage_axis_info(self, pos_min, pos_max, pitch, unit=1):
        ...
        if unit == 1:
            ...
            raise Exception(
                "The unit in method set_stage_axis_info is invalid! "
        ...
        self.aptdll.MOT_SetStageAxisInfo(self.SerialNum, pos_min_c, pos_max_c, unit_c, pitch_c)
    ...
    def getVelocityParameterLimits(self):
        ...
        if self._unit == "m":
            velocityParameterLimits = [
                maximumAcceleration.value / 1000.0,
                maximumVelocity.value / 1000.0,
            ]
        else:
    ...
    def get_home_parameter(self):
        ...
        zero_offset = c_float()
        self.aptdll.MOT_GetHomeParams(
            self.SerialNum,
            pointer(home_direction),
    ...
    def set_home_parameter(self, home_dir, switch_dir, home_vel, zero_offset):
        ...
        self.aptdll.MOT_SetHomeParams(
            self.SerialNum, home_dir_c, switch_dir_c, home_vel_c, zero_offset_c
        )
...
class APTStage(Base, MotorInterface):
    ...
    def on_activate(self):
        ...
        # Otherwise, look in the "standard form" thirdparty directory
            ...
            if platform.architecture()[0] == "64bit" or platform.architecture()[0] == "32bit":
                path_dll = os.path.join(
                    get_main_dir(), "thirdparty", "thorlabs", "win64", "APT.dll"
                )
            else:
        ...
        for axis_label in axis_label_list:
            ...
            self._axis_dict[axis_label] = APTMotor(path_dll, serialnumber, hw_type, label, unit)
            self._axis_dict[axis_label].initializeHardwareDevice()

            # adapt the hardware controller to the proper unit set:
            if (
                hw_conf_dict[axis_label]["unit"] == "°"
                or hw_conf_dict[axis_label]["unit"] == "degree"
            ):
    ...
    def get_constraints(self):
        ...
        for axis_label in config["axis_labels"]:
            ...
            if "pos_min" in constraintsconfig:
                ...
                self.log.warning(
                    "aptmotor has no pos_min specified in config file,using default value of 0."
                )
                this_axis["pos_min"] = 0
            ...
            if "pos_max" in constraintsconfig:
                ...
                self.log.warning(
                    "aptmotor has no pos_max specified in config file,using default value of 360."
                )
            ...
            if "vel_min" in constraintsconfig:
                ...
                self.log.warning(
                    "aptmotor has no vel_min specified in config file,using default value of 0.1."
                )
            ...
            if "vel_max" in constraintsconfig:
                ...
                self.log.warning(
                    "aptmotor has no vel_max specified in config file,using default value of 5.0."
                )
            ...
            if "acc_min" in constraintsconfig:
                ...
                self.log.warning(
                    "aptmotor has no acc_min specified in config file,using default value of 4.0."
                )
            ...
            if "acc_max" in constraintsconfig:
                ...
                self.log.warning(
                    "aptmotor has no acc_max specified in config file,using default value of 5.0."
                )
    ...
    def _save_pos(self, param_dict):
        ...
        for label_axis in param_dict:
            if label_axis in self._axis_dict:
                ...
                filename = os.path.join(self._magnet_dump_folder, label_axis + ".dat")
    ...
    def _get_magnet_dump(self):
        ...
        if not os.path.exists(magnet_path):
            ...
            self.log.info(f"Magnet dump was created in:\n{magnet_path}")
        return magnet_path
    ...
    def _get_config(self):
        ...
        for axis_label in config["axis_labels"]:
            ...
            if "serial_num" in axis_config:
                ...
                self.log.error(
                    f"No serial number given for APTmotor stage axis {axis_label}."
                    "It is impossible to proceed without this information."
                )
                break
            ...
            if "hw_type" in axis_config:
                ...
                self.log.warning(
                    f"No motor type given for APTmotor stage axis {axis_label}."
            ...
            if "pitch" in axis_config:
                ...
                self.log.warning(
                    "APTmotor has no pitch provided in config file."
            ...
            if "unit" in axis_config:
                ...
                self.log.warning(
                    "APTmotor has no unit specified in config file,taking degree by default."
                )
