...
class ConfocalScannerDummy(Base, ConfocalScannerInterface):
    ...
    def on_activate(self):
        ...
        self._points[:, 1] = np.random.uniform(
            self._position_range[0][0], self._position_range[0][1], self._num_points
        )
        # y_zero
        self._points[:, 2] = np.random.uniform(
            self._position_range[1][0], self._position_range[1][1], self._num_points
        )
        # sigma_x
    ...
    def set_position_range(self, myrange=None):
        ...
        for pos in myrange:
            if len(pos) != 2:
                self.log.error(
                    f"Given range limit {pos:d} should have "
                    f"dimension 2, but has {len(pos):d} instead."
                )
                return -1
            if pos[0] > pos[1]:
    ...
    def set_voltage_range(self, myrange=None):
        ...
        if myrange[0] > myrange[1]:
            self.log.error(f"Given range limit {myrange:d} has the wrong order.")
            return -1
    ...
    def scan_line(self, line_path=None, pixel_clock=False):
        ...
        for i in range(self._num_points):
            count_data += self.twoD_gaussian_function(
                (x_data, y_data), *(self._points[i])
            ) * self.gaussian_function(np.array(z_data), *(self._points_z[i]))

        time.sleep(self._line_length * 1.0 / self._clock_frequency)
        time.sleep(self._line_length * 1.0 / self._clock_frequency)
        ...
        return np.array(
            [count_data, 5e5 - count_data, np.ones(count_data.shape) * line_path[1, 0] * 100]
        ).transpose()
