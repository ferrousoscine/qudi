...
class TimeTaggerCounter(Base, SlowCounterInterface):
    ...
    def set_up_counter(
        ...
        # set at startup
        if self._mode == 1:
            channel_combined = tt.Combiner(
                self._tagger, channels=[self._channel_apd_0, self._channel_apd_1]
            )
        ...
        self.log.info(f"set up counter with {self._count_frequency}")
        return 0
    ...
    def get_counter(self, samples=None):
        ...
        if self._mode < 2:
            ...
            return np.array(
                [
                    self.counter0.getData() * self._count_frequency,
