...
IS_SENSOR_C2057R12M_WO = (
    0x0044  #  cmos, 2048x1536, rolling, 1/2", mono,  USB board (special version WO)
)
IS_SENSOR_C2057R12C_WO = (
    0x0045  #  cmos, 2048x1536, rolling, 1/2", color, USB board (special version WO)
)

IS_SENSOR_C2597R12M = (
    0x0048  #  cmos, 2592x1944, rolling, 1/2", mono,  USB board (special version WO)
)
IS_SENSOR_C2597R12C = (
    0x0049  #  cmos, 2592x1944, rolling, 1/2", color, WO board (special version WO)
)
...
IS_NETWORK_FRAME_SIZE_INCOMPATIBLE = (
    202  #  The frame size settings of the device and the network adapter are incompatible
)
IS_NETWORK_CONFIGURATION_INVALID = 203  #  The network adapter configuration is invalid
IS_ERROR_CPU_IDLE_STATES_CONFIGURATION = (
    204  #  The setting of the CPU idle state configuration failed
)
...
class UC480_CAPTURE_ERROR_INFO(ctypes.Structure):
    ...
    _fields_ = [
        ("dwCapErrCnt_Total", wt.DWORD),
        ("reserved", wt.BYTE * 60),
        ("adwCapErrCnt_Detail", wt.DWORD * 256),
    ]  #: Attributes
...
def create_camera_list(dwCount):
    ...
    class UC480_CAMERA_LIST(ctypes.Structure):
        _fields_ = [("dwCount", wt.ULONG), ("uci", UC480_CAMERA_INFO * dwCount)]
...
def IS_CAMERA_AVAILABLE(_s_):
    return (
        ((_s_ & FIRMWARE_DOWNLOAD_NOT_SUPPORTED) == 0)
...
class AUTO_WB_CHANNNEL_STATUS(ctypes.Structure):
    ...
    _fields_ = [("curValue", wt.DWORD), ("curError", ctypes.c_long), ("curCtrlStatus", wt.DWORD)]
...
class KNEEPOINTARRAY(ctypes.Structure):
    ...
    _fields_ = [("NumberOfUsedKneepoints", ctypes.c_int), ("Kneepoint", KNEEPOINT * 10)]
...
IS_SE_STARTER_FW_UPLOAD = (
    0x00000001  # !< get estimated duration of GigE SE starter firmware upload in milliseconds
)
IS_CP_STARTER_FW_UPLOAD = (
    0x00000002  # !< get estimated duration of GigE CP starter firmware upload in milliseconds
)
IS_STARTER_FW_UPLOAD = (
    0x00000004  # !< get estimated duration of starter firmware upload in milliseconds using hCam to
)
...
class UC480_ETH_ADDR_IPV4_by(ctypes.Structure):
    ...
    _fields_ = [("by1", wt.BYTE), ("by2", wt.BYTE), ("by3", wt.BYTE), ("by4", wt.BYTE)]
...
class UC480_ETH_ADDR_IPV4(ctypes.Structure):
    ...
    _fields_ = [("by", UC480_ETH_ADDR_IPV4_by), ("dwAddr", wt.DWORD)]
...
IS_ETH_CTRLSTATUS_ACCESSIBLE1 = (
    0x00000002  # !< device is accessible BY US, i.e. directly 'unicastable'
)
...
IS_ETH_CTRLSTATUS_PERSISTENT_IP_USED = (
    0x00000010  # !< device is running on persistent ip configuration
)
...
IS_ETH_CTRLSTATUS_ADAPTER_SETUP_OK = (
    0x00000080  # !< adapter's setup is ok with respect to uc480 needs
)
...
class UC480_ETH_ETHERNET_CONFIGURATION(ctypes.Structure):
    ...
    _fields_ = [("ipcfg", UC480_ETH_IP_CONFIGURATION), ("mac", UC480_ETH_ADDR_MAC)]
...
class UC480_ETH_DRIVER_INFO(ctypes.Structure):
    ...
    _fields_ = [
        ("dwMinVerStarterFirmware", wt.DWORD),
...
class IS_DEVICE_INFO_CONTROL(ctypes.Structure):
    ...
    _fields_ = [("dwDeviceId", wt.DWORD), ("reserved", wt.BYTE * 148)]
...
class OPENGL_DISPLAY(ctypes.Structure):
    ...
    _fields_ = [("nWindowID", ctypes.c_int), ("pDisplay", ctypes.c_void_p)]
...
IMGFRMT_CMD_GET_ARBITRARY_AOI_SUPPORTED = (
    4  #  Does the device supports the setting of an arbitrary AOI.
)
...
TRIGGER_DEBOUNCE_CMD_GET_DELAY_TIME_MIN = (
    5  #  Get the minimum value for the trigger debounce delay time
)
TRIGGER_DEBOUNCE_CMD_GET_DELAY_TIME_MAX = (
    6  #  Get the maximum value for the trigger debounce delay time
)
...
class IS_POINT_2D(ctypes.Structure):
    ...
    _fields_ = [("s32X", wt.INT), ("s32Y", wt.INT)]
...
class IS_SIZE_2D(ctypes.Structure):
    ...
    _fields_ = [("s32Width", wt.INT), ("s32Height", wt.INT)]
...
class IS_RECT(ctypes.Structure):
    ...
    _fields_ = [("s32X", wt.INT), ("s32Y", wt.INT), ("s32Width", wt.INT), ("s32Height", wt.INT)]
...
def create_bootboost_idlist(numberOfEntries):
    ...
    class IS_BOOTBOOST_IDLIST(ctypes.Structure):
        _fields_ = [("u32NumberOfEntries", wt.DWORD), ("aList", IS_BOOTBOOST_ID * numberOfEntries)]
...
IS_CONFIG_CMD_GET_CAPABILITIES = (
    1  # !< Get supported configuration capabilities (bitmask of CONFIGURATION_CAPS)
)

IS_CONFIG_CPU_IDLE_STATES_CMD_GET_ENABLE = (
    2  # !< Get the current CPU idle states enable state (bitmask of CONFIGURATION_SEL)
)
...
IS_CONFIG_CPU_IDLE_STATES_CMD_GET_DISABLE_ON_OPEN = (
    5  # !< Get the current setting of the command IS_CPU_IDLE_STATES_CMD_SET_DISABLE_ON_OPEN
)
...
IS_CONFIG_CPU_IDLE_STATES_CAP_SUPPORTED = (
    0x00000001  # !< CPU idle state commands are supported by the SDK
)
...
IS_CONFIG_INITIAL_PARAMETERSET_CAP_SUPPORTED = (
    0x00000004  # !< Initial parameter set commands are supported by the SDK
)
...
class IO_FLASH_PARAMS(ctypes.Structure):
    ...
    _fields_ = [("s32Delay", wt.INT), ("u32Duration", wt.UINT)]
...
class IO_PWM_PARAMS(ctypes.Structure):
    ...
    _fields_ = [("dblFrequency_Hz", ctypes.c_double), ("dblDutyCycle", ctypes.c_double)]
...
IO_FLASH_GPIO_PORT_MASK = (
    IO_FLASH_MODE_GPIO_1 | IO_FLASH_MODE_GPIO_2 | IO_FLASH_MODE_GPIO_3 | IO_FLASH_MODE_GPIO_4
)
...
class IS_RANGE_S32(ctypes.Structure):
    ...
    _fields_ = [("s32Min", wt.INT), ("s32Max", wt.INT), ("s32Inc", wt.INT)]
...
class MEASURE_SHARPNESS_AOI_INFO(ctypes.Structure):
    ...
    _fields_ = [("u32NumberAOI", wt.UINT), ("u32SharpnessValue", wt.UINT), ("rcAOI", IS_RECT)]
...
class ID_RANGE(ctypes.Structure):
    ...
    _fields_ = [("u32First", wt.UINT), ("u32Last", wt.UINT)]
...
class IMGBUF_ITERATION_INFO(ctypes.Structure):
    ...
    _fields_ = [("u32IterationID", wt.UINT), ("u32ImageID", wt.UINT)]
