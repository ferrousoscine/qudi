...
class CameraThorlabs(Base, CameraInterface):
    ...
    def _check_int_range(self, value, mini, maxi, message):
        ...
        if value < mini or value > maxi:
            self.log.error(f"{message} - Value {value} must be between {mini} and {maxi}")
            return False
        else:
            return True
    ...
    def _connect_camera(self):
        ...
        if number_of_cameras.value < 1:
            ...
            self.log.debug(f"Connected to camera : {str(self._sensor_info.strSensorName)}")
    ...
    def set_image_size(self, width=None, height=None):
        ...
        if height is not None:
            ...
            self._check_int_range(
                height, 1, self._sensor_info.nMaxHeight, "Can not set image height"
            )
            self._height = height

        code = self._dll.is_SetImageSize(
            self._camera_handle, ctypes.c_int(self._width), ctypes.c_int(self._height)
        )
    ...
    def set_image_position(self, pos_x, pos_y):
        ...
        if pos_x is not None:
            pos_x = int(pos_x)
            self._check_int_range(
                pos_x, 0, self._sensor_info.nMaxWidth - 1, "Can not set image position x"
            )
            self._pos_x = pos_x
        if pos_y is not None:
            pos_y = int(pos_y)
            self._check_int_range(
                pos_y, 0, self._sensor_info.nMaxHeight - 1, "Can not set image position y"
            )
            self._pos_y = pos_y

        code = self._dll.is_SetImagePos(
            self._camera_handle, ctypes.c_int(self._pos_x), ctypes.c_int(self._pos_y)
        )
    ...
    def get_acquired_data(self):
        ...
        code = self._dll.is_CopyImageMem(
            self._camera_handle, self._image_memory, self._image_pid, c_img
        )
