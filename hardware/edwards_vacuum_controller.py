...
class EdwardsVacuumController(Base):
    ...
    PRIORITY = {0: "OK", 1: "Warning", 2: "Alarm", 3: "Alarm"}
    ...
    GAUGE_UNIT = {66: "Voltage", 59: "Pressure", 81: "Percent"}
    ...
    def _get_pstate(self, register):
        ...
        """
        g = self.inst.ask(f"?V{register}")
        param = g.split()[0]
        value = g.split()[1]
        if param == f"=V{register}":
            values = value.split(";")
            parsed = {
    ...
    def get_pump_speeds(self):
        """Get pump speeds."""
        return {
            "turbo": self.get_turbo_speed()["value"],
            "backing": self.get_backing_speed()["value"],
        }

    def get_pump_powers(self):
        """Get pump powers."""
        return {
            "turbo": self.get_turbo_power()["value"],
            "backing": self.get_backing_power()["value"],
        }

    def get_pump_states(self):
        """Get pump states."""
        return {
            "turbo": self.get_turbo_status()["state"],
            "backing": self.get_backing_status()["state"],
        }
    ...
    def set_system_state(self, state):
        """Set system state"""
        return self.inst.ask(f"!C933 {1 if state else 0}")
