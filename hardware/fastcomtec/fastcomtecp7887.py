...
class FastComtec(Base, FastCounterInterface):
    ...
    def configure(self, bin_width_s, record_length_s, number_of_gates=0, filename=None):
        ...
        if self.gated:
            ...
            no_of_bins = int(
                (record_length_FastComTech_s + self.aom_delay) / self.set_binwidth(bin_width_s)
            )
        else:
            # subtract time to make sure no sequence trigger is missed
            no_of_bins = int(
                (record_length_FastComTech_s - self.trigger_safety) / self.set_binwidth(bin_width_s)
            )
    ...
    def get_data_trace(self):
        ...
        info_dict = {"elapsed_sweeps": self.get_current_sweeps(), "elapsed_time": None}
    ...
    def set_length(self, length_bins, preset=None, cycles=None, sequences=None):
        ...
        if length_bins * self.get_binwidth() < constraints["max_sweep_len"]:
            cmd = f"RANGE={int(length_bins)}"
            self.dll.RunCmd(0, bytes(cmd, "ascii"))
            cmd = f"roimax={int(length_bins)}"
            self.dll.RunCmd(0, bytes(cmd, "ascii"))
            if preset is not None:
                cmd = f"swpreset={preset}"
                self.dll.RunCmd(0, bytes(cmd, "ascii"))
            if cycles:
                cmd = f"cycles={cycles}"
                self.dll.RunCmd(0, bytes(cmd, "ascii"))
            if sequences:
                cmd = f"sequences={sequences}"
                self.dll.RunCmd(0, bytes(cmd, "ascii"))
    ...
    def change_sweep_mode(self, gated):
        if gated:
            ...
            cmd = f"sweepmode={hex(number)}"
            self.dll.RunCmd(0, bytes(cmd, "ascii"))
            cmd = f"prena={hex(16)}"  # To select starts preset
    ...
    def SaveData_locally(self, filename, laser_index):
        ...
        for i in laser_index:
            for n in data[
                i : i
                + int(round(3000 / (self.minimal_binwidth * 2 ** self.GetBitshift())))
