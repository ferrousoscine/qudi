...
class FastComtec(Base, FastCounterInterface):
    ...
    def on_activate(self):
        ...
        self.dll = ctypes.windll.LoadLibrary(r"C:\Windows\System32\DMCS6.dll")
        if self.gated:
    ...
    def set_length(self, length_bins):
        ...
        if length_bins * cycles < constraints["max_bins"]:
            ...
            self.log.error(f"Dimensions {length_bins * cycles} are too large for fast counter1!")
            return -1
    ...
    def set_cycles(self, cycles):
        ...
        if self.get_length() * cycles < constraints["max_bins"]:
            ...
            self.log.error(
                f"Dimensions {self.get_length() * cycles} are too large for fast counter2!"
            )
            return -1
