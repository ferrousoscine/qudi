...
class MicrowaveSmbv(Base, MicrowaveInterface):
    ...
    def on_activate(self):
        ...
        try:
            ...
            self.log.error(f"Could not connect to the address >>{self._address}<<.")
            raise

        self.model = self._connection.query("*IDN?").split(",")[1]
        self.log.info(f"MW {self.model} initialised and connected.")
    ...
    def set_sweep(self, start=None, stop=None, step=None, power=None):
        ...
        if (start is not None) and (stop is not None) and (step is not None):
            ...
            self._connection.write("*WAI")
            self._connection.write(f":FREQ:START {start - step:f}")
            self._connection.write(f":FREQ:STOP {stop:f}")
            self._connection.write(f":SWE:STEP:LIN {step:f}")
            self._connection.write("*WAI")

        if power is not None:
            self._connection.write(f":POW {power:f}")
            self._connection.write("*WAI")
