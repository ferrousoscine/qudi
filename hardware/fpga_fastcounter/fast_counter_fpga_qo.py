...
class FastCounterFPGAQO(Base, FastCounterInterface):
    ...
    __error_encoding = {
        0x00000020: "Init/output FSM in FPGA hardware encountered an error."
    ...
    def _connect(self):
        ...
        bitfile_name = f"fastcounter_{self._fpga_type}.bit"
        ...
        if not self._fpga.IsFrontPanelEnabled():
            self.log.error(
                "Opal Kelly FrontPanel is not enabled in FPGA.\nUpload of bitfile failed."
            )
            self._statusvar = -1
            return -1
        ...
        while True:
            if time.time() - start_time >= timeout:
                self.log.error(
                    "Power-on initialization of FPGA-timetagger timed out. Device non-functional."
                )
                self._statusvar = -1
                break
    ...
    def start_measure(self):
        ...
        with self.threadlock:
            ...
            self.count_data = np.zeros(
                [self._number_of_gates, self._gate_length_bins], dtype="int64"
            )
            # Start the counter.
        ...
        with self.threadlock:
            ...
            if len(status_messages) != 1 or ("running" not in status_messages):
                self.log.error(
                    ...
                    f"trace of the device. An empty numpy array[{self._number_of_gates},{self._gate_length_bins}] filled with "
            ...
            if read_err_code != buffersize:
                self.log.error(
                    f"Data transfer from FPGA via USB failed with error code {read_err_code}. "
                    "Returning old count data."
                )
                return self.count_data, info_dict
            ...
            buffer_encode = buffer_encode.reshape(512, 65536)[
                0 : self._number_of_gates, 0 : self._gate_length_bins
            ]
    ...
    def stop_measure(self):
        ...
        with self.threadlock:
            ...
            while True:
                status_messages = self._get_status_messages()
                if (
                    len(status_messages) == 2
                    and ("idle_ready" in status_messages)
                    and ("TDC_in_reset" in status_messages)
                ):
