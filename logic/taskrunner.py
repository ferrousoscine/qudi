...
class TaskRunner(GenericLogic):
    ...
    def loadTasks(self):
        ...
        for task in config["tasks"]:
            ...
            # print('tsk:', task)
            if "module" not in config["tasks"][task]:
                self.log.error(f"No module given for task {task}")
                continue
            else:
            ...
            try:
                ...
                # print('dir:', dir(mod))
                t["object"] = mod.Task(
                    name=t["name"], runner=self, references=ref, config=t["config"]
                )
                if isinstance(t["object"], gt.InterruptableTask) or isinstance(
                    t["object"], gt.PrePostTask
                ):
                    self.model.append(t)
                else:
    ...
    def checkTasksInModel(self):
        ...
        for task in self.model.storage:
            ...
            for _moddef, mod in task["needsmodules"].items():
                ...
                if mod in self._manager.tree["loaded"]["logic"] and not self._manager.tree[
                    "loaded"
                ]["logic"][mod].module_state.isstate("deactivated"):
                    modok = True
    ...
    def startTask(self, task):
        ...
        if not task["ok"]:
            self.log.error(
                "Task {} did not pass all checks for required "
                "tasks and modules and cannot be run".format(task["name"])
            )
            return
        if task["object"].can("run"):
            ...
            self.log.error(f"Task cannot be run: {task.name}")
                ...
                for t in self.model.storage:
                    if t["name"] == ptask:
                        if t["object"].can("resume"):
                            ...
                            self.log.error(
                                "Pausetask {} failed while resuming after stop: {}".format(
                                    ptask, task["name"]
                                )
                            )
                            return False
                ...
                for t in self.model.storage:
                    if t["name"] == pptask:
                        if t["object"].can("postrun"):
                            ...
                            self.log.error(
                                "Preposttask {} failed while postrunning in: {}".format(
                                    pptask, task["name"]
                                )
                            )
                            return False
                ...
                for t in self.model.storage:
                    if t["name"] == pptask:
                        if t["object"].can("prerun"):
                            ...
                            self.log.error(
                                "Preposttask {} failed while preparing: {}".format(
                                    pptask, task["name"]
                                )
                            )
                            return False
                ...
                for t in self.model.storage:
                    if t["name"] == ptask:
                        if t["object"].can("pause"):
                            ...
                            self.log.error(
                                "Pausetask {} failed while preparing: {}".format(
                                    ptask, task["name"]
                                )
                            )
                            return False
