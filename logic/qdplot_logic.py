...
class QDPlotLogic(GenericLogic):
    ...
    def remove_plot(self, plot_index=None):
        with self.threadlock:
            ...
            for i in update_range:
                self.sigPlotDataUpdated.emit(
                    i, self._x_data[i], self._y_data[i], self._clear_old[i]
                )
                self.sigFitUpdated.emit(
                    i, self._fit_data[i], self._fit_results[i], self._fit_method[i]
                )
                params = {
    ...
    def get_x_data(self, plot_index=0):
        ...
        with self.threadlock:
            ...
            self.log.error(
                f"Error while retrieving plot x_data. Plot index {plot_index:d} out of bounds."
            )
            return [np.zeros(0)]
    ...
    def get_y_data(self, plot_index=0):
        ...
        with self.threadlock:
            ...
            self.log.error(
                f"Error while retrieving plot y_data. Plot index {plot_index:d} out of bounds."
            )
            return [np.zeros(0)]
    ...
    def set_data(self, x=None, y=None, clear_old=True, plot_index=0, adjust_scale=True):
        ...
        with self.threadlock:
            ...
            if not (0 <= plot_index < self.number_of_plots):
                self.log.error(
                    f"Plot index {plot_index:d} out of bounds. To add a new plot, call set_number_of_plots(int) "
                    "or add_plot() first."
                )
                return -1
            ...
            self.sigPlotDataUpdated.emit(
                plot_index, self._x_data[plot_index], self._y_data[plot_index], clear_old
            )
            self.sigPlotParamsUpdated.emit(
                plot_index,
                {"x_limits": self._x_limits[plot_index], "y_limits": self._y_limits[plot_index]},
            )
            if adjust_scale:
    ...
    def do_fit(self, fit_method, plot_index=0):
        ...
        with self.threadlock:
            if not (0 <= plot_index < self.number_of_plots):
                raise IndexError(
                    f"Plot index {plot_index:d} out of bounds. Unable to perform data fit."
                )
            ...
            if fit_method is None or isinstance(fit_method, str):
                if fit_method not in self.fit_container.fit_list:
                    if fit_method is not None and fit_method != "No Fit":
                        self.log.warning(
                            f'Fit function "{fit_method}" not available in fit container. Configure '
                            "available fits first."
                        )
                    fit_method = "No Fit"
            else:
    ...
    def save_data(self, postfix="", plot_index=0):
        ...
        with self.threadlock:
            ...
            for data_set in range(len(self._x_data[plot_index])):
                ax1.plot(
                    self._x_data[plot_index][data_set],
                ...
                if self._fit_data[plot_index] is not None:
                    ax1.plot(
                        self._fit_data[plot_index][data_set][0],
            ...
            if self._fit_data[plot_index] is not None:
                ...
                for column in chunks:
                    ...
                    heading = (
                        f"Fit results for method: {self._fit_method[plot_index]}"
                        if is_first_column
                        else ""
                    )
                    column_text = heading + "\n" + column_text
            ...
            plt.close(fig)
            self.log.debug(f"Data saved to:\n{file_path}")
        ...
        with self.threadlock:
            ...
            if limits is not None:
                if isinstance(limits, (list, tuple, np.ndarray)) and len(limits) > 1:
                    ...
                    self.log.error(
                        f"limits need to be a list of at least 2 elements but is {limits}."
                    )
                    return
            else:
