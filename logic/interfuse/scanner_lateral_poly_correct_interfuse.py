...
class ScannerLateralPolyCorrectInterfuse(GenericLogic, ConfocalScannerInterface):
    ...
    def on_activate(self):
        ...
        corners = [
            (self.config_range_x[0], self.config_range_y[0]),
        ...
        for corner in corners:
            ...
            if not (scanner_range[0][0] <= x <= scanner_range[0][1]) or not (
                scanner_range[1][0] <= x <= scanner_range[1][1]
            ):
                self.log.error(
                    "Provided range does not match polynomes. Corner ({}, {}) transformed to ({}, {}) is not"
    ...
    def set_up_scanner(
        ...
        return self.scanner().set_up_scanner(
    ...
    def scanner_set_position(self, x=None, y=None, z=None, a=None):
        ...
        ):  # after activation, if logic has not setted position once
            self._position[0] = x
            self._position[1] = y
