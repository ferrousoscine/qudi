    ...
    def scan_line(self, line_path=None, pixel_clock=False):
        ...
        if line_path[0, 0] != line_path[0, 1]:
            ...
            for i in range(self._num_points):
                count_data += self._fit_logic.twoD_gaussian_function(
                    (x_data, y_data), *(self._points[i])
                ) * (self._fit_logic.gaussian_function(np.array(z_data[0]), *(self._points_z[i])))
        else:
            x_data, y_data = np.meshgrid(line_path[0, 0], line_path[1, 0])
            for i in range(self._num_points):
                count_data += self._fit_logic.twoD_gaussian_function(
                    (x_data, y_data), *(self._points[i])
                ) * (self._fit_logic.gaussian_function(z_data, *(self._points_z[i])))
