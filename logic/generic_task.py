...
class InterruptableTask(QtCore.QObject, Fysom, metaclass=TaskMetaclass):
    ...
    def _doStart(self):
        ...
        try:
            ...
            self.log.exception(f"Exception during task {self.name}. {e}")
    ...
    def _doTaskStep(self):
        ...
        try:
            ...
            self.log.exception(f"Exception during task step {self.name}. {e}")
            self.result.update(None, False)
            self.finish()
            self.sigDoFinish.emit()
    ...
    def _doPause(self):
        ...
        try:
            ...
            self.log.exception(f"Exception while pausing task {self.name}. {e}")
            self.result.update(None, False)
    ...
    def _doResume(self):
        ...
        try:
            ...
            self.log.exception(f"Exception while resuming task {self.name}. {e}")
            self.result.update(None, False)
    ...
    def checkStartPrerequisites(self):
        ...
        for task in self.prePostTasks:
            if not (
                isinstance(self.prePostTasks[task], PrePostTask)
                and self.prePostTasks[task].can("prerun")
            ):
                self.log(
                    f"Cannot start task {self.name} as pre/post task {task} is not in a state to run.",
                    msgType="error",
                )
                return False
        for task in self.pauseTasks:
            if not (
                isinstance(self.pauseTasks[task], InterruptableTask)
                and (self.pauseTasks[task].can("pause") or self.pauseTasks[task].isstate("stopped"))
            ):
                self.log(
                    f"Cannot start task {self.name} as interruptable task {task} is not stopped or able to pause.",
                    msgType="error",
                )
                return False
    ...
    def checkPausePrerequisites(self):
        ...
        try:
            ...
            self.log.exception(
                f"Exception while checking pause prerequisites for task {self.name}. {e}"
            )
            return False
