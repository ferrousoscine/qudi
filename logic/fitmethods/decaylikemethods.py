...
def make_barestretchedexponentialdecay_model(self, prefix=None):
    ...
    if not isinstance(prefix, str) and prefix is not None:
        ...
        model = Model(barestretchedexponentialdecay_function, independent_vars=["x"], prefix=prefix)
...
def estimate_decayexponential(self, x_axis, data, params):
    ...
    try:
        ...
        linear_result = self.make_linear_fit(
            x_axis=x_axis[0:i], data=data_level_log, estimator=self.estimate_linear
        )
...
def make_decayexponentialstretched_fit(
    ...
    result_str_dict["Beta"] = {
        ...
        "unit": "",
    }  # Beta (exponent of exponential exponent)
...
def make_biexponential_fit(self, x_axis, data, estimator, units=None, add_params=None, **kwargs):
    ...
    result_str_dict["1st amplitude"] = {
        "value": result.params["e0_amplitude"].value,
        "error": result.params["e0_amplitude"].stderr,
        "unit": units[1],
    }  # amplitude

    result_str_dict["1st lifetime"] = {
        "value": result.params["e0_lifetime"].value,
        "error": result.params["e0_lifetime"].stderr,
        "unit": units[0],
    }  # lifetime

    result_str_dict["1st beta"] = {
        "value": result.params["e0_beta"].value,
        "error": result.params["e0_beta"].stderr,
        "unit": "",
    }  # Beta (exponent of exponential exponent)

    result_str_dict["2nd amplitude"] = {
        "value": result.params["e1_amplitude"].value,
        "error": result.params["e1_amplitude"].stderr,
        "unit": units[1],
    }  # amplitude

    result_str_dict["2nd lifetime"] = {
        "value": result.params["e1_lifetime"].value,
        "error": result.params["e1_lifetime"].stderr,
        "unit": units[0],
    }  # lifetime

    result_str_dict["2nd beta"] = {
        "value": result.params["e1_beta"].value,
