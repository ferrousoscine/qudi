...
class NuclearOperationsLogic(GenericLogic):
    ...
    def initialize_x_axis(self):
        ...
        self.x_axis_list = np.arange(
            self.x_axis_start, stop + (self.x_axis_step / 2), self.x_axis_step
        )
        self.current_meas_point = self.x_axis_start
    ...
    def _meas_point_loop(self):
        ...
        if self.next_optimize_time < self.elapsed_time:
            ...
            if self.mw_on_odmr_peak == 1:
                ...
                self.log.error(
                    ...
                    f'be selected but an number of "{self.mw_on_odmr_peak}" was set. '
                    "Measurement stopped!"
                )
        ...
        # results:
        self._set_meas_point(
            num_of_meas_runs=self.num_of_current_meas_runs,
    ...
    def _get_meas_point(self, meas_type):
        ...
        name_tag = f"{self.current_meas_asset_name}_{self.current_meas_point}"
        ...
        elif meas_type in ["QSD_-_Artificial_Drive", "QSD_-_SWAP_FID", "QSD_-_Entanglement_FID"]:
    ...
    def get_meas_type_list(self):
        return [
            "Nuclear_Rabi",
            "Nuclear_Frequency_Scan",
            "QSD_-_Artificial_Drive",
            "QSD_-_SWAP_FID",
    ...
    def prepare_measurement_protocols(self, meas_type):
        ...
        if meas_type == "Nuclear_Rabi":
            ...
            # sample:
            self._seq_gen_logic.sample_pulse_sequence(
                sequence_name=meas_type, write_to_file=True, chunkwise=False
            )
            # upload:
            ...
            meas_type == "QSD_-_Artificial_Drive"
            or meas_type == "QSD_-_SWAP_FID"
            or meas_type == "QSD_-_Entanglement_FID"
        ):
            pass
    ...
    def adjust_measurement(self, meas_type):
        ...
        if meas_type == "Nuclear_Rabi":
            ...
            meas_type == "QSD_-_Artificial Drive"
            or meas_type == "QSD_-_SWAP_FID"
            or meas_type == "QSD_-_Entanglement_FID"
        ):
            pass
    ...
    def _create_laser_on(self):
        ...
        self._seq_gen_logic.generate_laser_on(
            name="Laser_On", laser_time_bins=3000, laser_channel=self.pulser_laser_ch
        )
    ...
    def do_optimize_odmr_freq(self):
        ...
        self.measured_odmr_list.append(
            [curr_time, self.odmr_meas_freq0, self.odmr_meas_freq1, self.odmr_meas_freq2]
        )
    ...
    def save_nuclear_operation_measurement(self, name_tag=None, timestamp=None):
        ...
        self._save_logic.save_data(
            data1, filepath=filepath, parameters=param, filelabel=filelabel1, timestamp=timestamp
        )

        self._save_logic.save_data(
            data2, filepath=filepath, filelabel=filelabel2, timestamp=timestamp
        )

        self._save_logic.save_data(
            data4, filepath=filepath, filelabel=filelabel4, timestamp=timestamp
        )
        ...
        self.log.info(f"Nuclear Operation data saved to:\n{filepath}")
