...
class OptimizerLogic(GenericLogic):
    ...
    def start_refocus(self, initial_pos=None, caller_tag="unknown", tag="logic"):
        ...
        if (
            isinstance(initial_pos, (np.ndarray,))
            and initial_pos.size >= 3
            or isinstance(initial_pos, (list, tuple))
            and len(initial_pos) >= 3
        ):
        ...
        if scanner_status < 0:
            self.sigRefocusFinished.emit(
                self._caller_tag, [self.optim_pos_x, self.optim_pos_y, self.optim_pos_z, 0]
            )
            return
    ...
    def _initialize_xy_refocus_image(self):
        ...
        self.xy_refocus_image = np.zeros(
            (len(self._Y_values), len(self._X_values), 3 + len(self.get_scanner_count_channels()))
        )
    ...
    def _set_optimized_xy_from_fit(self):
        ...
        result_2D_gaus = self._fit_logic.make_twoDgaussian_fit(
            xy_axes=axes, data=xy_fit_data, estimator=self._fit_logic.estimate_twoDgaussian_MLE
        )
        ...
        if result_2D_gaus.success is False:
            ...
            if (
                abs(self._initial_pos_x - result_2D_gaus.best_values["center_x"]) < self._max_offset
                and abs(self._initial_pos_x - result_2D_gaus.best_values["center_x"])
                < self._max_offset
            ):
    ...
    def finish_refocus(self):
        ...
        self.log.info(
            f"Optimised from ({self._initial_pos_x:.3e},{self._initial_pos_y:.3e},{self._initial_pos_z:.3e}) to local "
            f"maximum at ({self.optim_pos_x:.3e},{self.optim_pos_y:.3e},{self.optim_pos_z:.3e})."
        )
