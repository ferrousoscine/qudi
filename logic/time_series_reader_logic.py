...
class TimeSeriesReaderLogic(GenericLogic):
    ...
    def on_activate(self):
        ...
        if self._moving_average_width % 2 == 0:
            self.log.warning(
                ...
                f"Changing value from {self._moving_average_width:d} to {self._moving_average_width + 1:d}."
            )
    ...
    def trace_data(self):
        ...
        data = {
            ch: self._trace_data[i, :data_offset] for i, ch in enumerate(self.active_channel_names)
        }
    ...
    def averaged_trace_data(self):
        if not self.averaged_channel_names or self.moving_average_width <= 1:
            return None, None
        data = {
            ch: self._trace_data_averaged[i] for i, ch in enumerate(self.averaged_channel_names)
        }
    ...
    def configure_settings(self, settings_dict=None, **kwargs):
        ...
        with self.threadlock:
            ...
            if "oversampling_factor" in settings_dict:
                ...
                if new_val < 1:
                    self.log.error(
                        f"Oversampling factor must be integer value >= 1 (received: {new_val:d})."
                    )
                else:
                    ...
                    if not (min_val <= (new_val * data_rate) <= max_val):
                        if "data_rate" in settings_dict:
                            ...
                            self.log.error(
                                f"Oversampling factor to set ({new_val:d}) would cause "
            ...
            if "moving_average_width" in settings_dict:
                ...
                if new_val < 1:
                    self.log.error(
                        f"Moving average width must be integer value >= 1 (received: {new_val:d})."
                    )
                elif new_val % 2 == 0:
                    new_val += 1
                    self.log.warning(
                        "Moving average window must be odd integer number in order to "
                        f"ensure perfect data alignment. Will increase value to {new_val:d}."
                    )
                if new_val / data_rate > self.trace_window_size:
                    if "data_rate" in settings_dict or "trace_window_size" in settings_dict:
                        ...
                        self.log.warning(
                            f"Moving average width to set ({new_val:d}) is smaller than the "
                    ...
                    self._moving_average_width = new_val
                    self.__moving_filter = np.full(
                        shape=self.moving_average_width, fill_value=1.0 / self.moving_average_width
                    )
            ...
            if "data_rate" in settings_dict:
                ...
                if new_val < 0:
                    ...
                    if not (min_val <= sample_rate <= max_val):
                        self.log.warning(
                            f"Data rate to set ({new_val:.3e}Hz) would cause sampling rate "
                    ...
                    if self.moving_average_width / data_rate > self.trace_window_size:
                        if "trace_window_size" not in settings_dict:
                            self.log.warning(
                                f"Data rate to set ({new_val:.3e}Hz) would cause too few "
            ...
            if "trace_window_size" in settings_dict:
                ...
                if new_val < 0:
                    ...
                    if data_points < self.moving_average_width:
                        self.log.warning(
                            f"Requested trace_window_size ({new_val:.3e}s) would have too "
    ...
    def acquire_data_block(self):
        ...
        with self.threadlock:
            if self.module_state() == "locked":
                ...
                samples_to_read = max(
                    (self._streamer.available_samples // self._oversampling_factor)
                ...
                if data.shape[1] != samples_to_read:
                    self.log.error(
                        "Reading data from streamer went wrong; "
    ...
    def _process_trace_data(self, data):
        ...
        if self.oversampling_factor > 1:
            if data.shape[1] % self.oversampling_factor != 0:
                ...
                return -1
            tmp = data.reshape(
                (data.shape[0], data.shape[1] // self.oversampling_factor, self.oversampling_factor)
            )
            data = np.mean(tmp, axis=2)

        digital_channels = [
            c for c, typ in self.active_channel_types.items() if typ == StreamChannelType.DIGITAL
        ]
    ...
    def _save_recorded_data(self, to_file=True, name_tag="", save_figure=True):
        ...
        if to_file:
            ...
            filelabel = f"data_trace_{name_tag}" if name_tag else "data_trace"

            # prepare the data in a dict:
            header = ", ".join(f"{ch} ({unit})" for ch, unit in self.active_channel_units.items())
            ...
            self.log.info(f"Time series saved to: {filepath}")
    ...
    def _draw_figure(self, data, timebase, y_unit):
        ...
        if max_abs_value.scale:
            ax.plot(
                time_data, data.transpose() / max_abs_value.scale_val, linestyle=":", linewidth=0.5
            )
        else:
    ...
    def save_trace_snapshot(self, to_file=True, name_tag="", save_figure=True):
        ...
        with self.threadlock:
            ...
            if to_file:
                filepath = self._savelogic.get_path_for_module(module_name="TimeSeriesReader")
                filelabel = f"data_trace_snapshot_{name_tag}" if name_tag else "data_trace_snapshot"
                self._savelogic.save_data(
                    data=data,
                    filepath=filepath,
                    parameters=parameters,
                ...
                self.log.info(f"Time series snapshot saved to: {filepath}")
    ...
    def _stop_reader_wait(self):
        ...
        with self.threadlock:
            ...
            # terminate the hardware streaming
            if self._streamer.stop_stream() < 0:
                self.log.error("Error while trying to stop streaming device data acquisition.")
            if self._data_recording_active:
                self._save_recorded_data(to_file=True, save_figure=True)
                self._recorded_data = list()
            self._data_recording_active = False
