...
class QDPlotterGui(GUIBase):
    ...
    def on_activate(self):
        ...
        for index, color in enumerate(self._pen_color_list):
            if (isinstance(color, (list, tuple)) and len(color) == 3) or (
                ...
                self.log.warning(
                    f'The color was "{color}" but needs to be from this list: {self._allowed_colors} '
    ...
    def update_number_of_plots(self, count):
        ...
        while count > len(self._plot_dockwidgets):
            ...
            dockwidget = PlotDockWidget(f"Plot {index + 1:d}", self._mw)
    ...
    def pen_color_list(self, value):
        if not isinstance(value, (list, tuple)) or len(value) < 1:
            self.log.warning(
                f'The parameter pen_color_list needs to be a list of strings but was "{value}".'
    ...
    def update_data(self, plot_index, x_data=None, y_data=None, clear_old=None):
        ...
        if not (0 <= plot_index < len(self._plot_dockwidgets)):
            self.log.warning(f"Tried to update plot with invalid index {plot_index:d}")
            return
        ...
        for line, xd in enumerate(x_data):
            ...
            self._plot_curves[plot_index].append(
                dockwidget.plot_PlotWidget.plot(
                    pen=mkColor(pen_color), symbol="d", symbolSize=6, symbolBrush=mkColor(pen_color)
                )
            )
    ...
    def update_plot_parameters(self, plot_index, params=None):
        ...
        if "x_label" in params or "x_unit" in params:
            label = params.get("x_label")
            unit = params.get("x_unit")
            if label is None:
        ...
        if "y_label" in params or "y_unit" in params:
            label = params.get("y_label")
            unit = params.get("y_unit")
            if label is None:
    ...
    def fit_clicked(self, plot_index=0):
        ...
        current_fit_method = (
            self._plot_dockwidgets[plot_index].widget().fit_comboBox.getCurrentFit()[0]
        )
