...
class WavemeterLogGui(GUIBase):
    ...
    def on_activate(self):
        ...
        self.curve_data_points = pg.PlotDataItem(pen=pg.mkPen(palette.c1), symbol=None)

        self.curve_nm_counts = pg.PlotDataItem(
            pen=pg.mkPen(palette.c2, style=QtCore.Qt.DotLine), symbol=None
        )

        self.curve_hz_counts = pg.PlotDataItem(
            pen=pg.mkPen(palette.c6, style=QtCore.Qt.DotLine), symbol=None
        )

        self.curve_envelope = pg.PlotDataItem(
            pen=pg.mkPen(palette.c3, style=QtCore.Qt.DotLine), symbol=None
        )

        self.curve_fit = pg.PlotDataItem(pen=pg.mkPen(palette.c2, width=3), symbol=None)
        ...
        self._scatterplot = pg.ScatterPlotItem(
            size=10, pen=pg.mkPen(None), brush=pg.mkBrush(255, 255, 255, 20)
        )
    ...
    def updateData(self):
        ...
        self._mw.wavelengthLabel.setText(f"{self._wm_logger_logic.current_wavelength:,.6f} nm ")
        self._mw.autoMinLabel.setText(f"Minimum: {self._wm_logger_logic.intern_xmin:3.6f} (nm)   ")
        self._mw.autoMaxLabel.setText(f"Maximum: {self._wm_logger_logic.intern_xmax:3.6f} (nm)   ")

        x_axis = self._wm_logger_logic.histogram_axis
        x_axis_hz = 3.0e17 / x_axis - 6.0e17 / (
            self._wm_logger_logic.get_max_wavelength() + self._wm_logger_logic.get_min_wavelength()
        )
    ...
    def updateFit(self):
        ...
        if fit_name != "No Fit":
            self.curve_fit.setData(
                x=self._wm_logger_logic.wlog_fit_x, y=self._wm_logger_logic.wlog_fit_y
            )
    ...
    def save_clicked(self):
        ...
        filename = os.path.join(
            filepath, timestamp.strftime("%Y%m%d-%H%M-%S_wavemeter_log_thumbnail")
        )
