...
class ErrorDialog(QtWidgets.QDialog):
    ...
    def __init__(self, logWindow):
        ...
        self.setGeometry(round((screenWidth - 500) / 2), round((screenHeight - 100) / 2), 500, 100)
    ...
    def show(self, entry):
        ...
        while key in exc:
            ...
            if exc["message"].startswith("HelpfulException"):
                exceptions.append(
                    "<b>"
                    + self.cleanText(re.sub(r"^HelpfulException: ", "", exc["message"]))
                    + "</b>"
                )
        ...
        if self.isVisible():
            ...
            self.nextBtn.setText(f"Show next error ({len(self.messages):d} more)")
        else:
