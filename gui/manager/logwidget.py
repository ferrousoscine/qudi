...
class LogModel(QtCore.QAbstractTableModel):
    ...
    def headerData(self, section, orientation, role=QtCore.Qt.DisplayRole):
        ...
        """
        if (
            not (0 <= section < len(self.header))
            or role != QtCore.Qt.DisplayRole
            or orientation != QtCore.Qt.Horizontal
        ):
            return None
        else:
...
class LogFilter(QtCore.QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, sourceRow, sourceParent):
        ...
        level = self.sourceModel().data(indexLevel, QtCore.Qt.DisplayRole)
        if level is None:
            return False
    ...
    def lessThan(self, left, right):
        ...
        """
        leftData = self.sourceModel().data(
            self.sourceModel().index(left.row(), 0), QtCore.Qt.DisplayRole
        )
        rightData = self.sourceModel().data(
            self.sourceModel().index(right.row(), 0), QtCore.Qt.DisplayRole
        )
        ...
        if e.type() == QtCore.QEvent.ToolTip:
            ...
            if rect.width() < size.width():
                ...
                QtWidgets.QToolTip.showText(
                    e.globalPos(), f"<div>{html.escape(tooltip)}</div>", view
                )
            else:
...
class LogWidget(QtWidgets.QWidget):
    ...
    def __init__(self, manager=None, **kwargs):
        ...
        if qtpy.PYQT4 or qtpy.PYSIDE:
            self.output.horizontalHeader().setResizeMode(0, QtWidgets.QHeaderView.Interactive)
            self.output.horizontalHeader().setResizeMode(1, QtWidgets.QHeaderView.ResizeToContents)
            self.output.horizontalHeader().setResizeMode(2, QtWidgets.QHeaderView.ResizeToContents)
            self.output.horizontalHeader().setResizeMode(3, QtWidgets.QHeaderView.ResizeToContents)
            self.output.verticalHeader().setResizeMode(QtWidgets.QHeaderView.ResizeToContents)
        else:
