        ...
        *.ui file and configures the event handling between the modules.
        ...
        if self._no_logic.current_meas_asset_name != "":
            index = self._mw.current_meas_asset_name_ComboBox.findText(
                self._no_logic.current_meas_asset_name, QtCore.Qt.MatchFixedString
            )
            if index >= 0:
    ...
    def current_meas_asset_name_changed(self):
        ...
        if name == "Nuclear_Rabi":
            ...
            self._mw.nulcear_ops_GraphicsView.setLabel(
                axis="bottom", text="RF pulse length", units="s"
            )
            self._mw.nulcear_ops_GraphicsView.setLabel(axis="left", text="Flip probability")
            ...
            self._mw.nulcear_ops_GraphicsView.setLabel(
                axis="bottom", text="Pulse length", units="s"
            )
            self._mw.nulcear_ops_GraphicsView.setLabel(axis="left", text="Flip probability")
