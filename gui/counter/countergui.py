...
class CounterGui(GUIBase):
    ...
    def on_activate(self):
        ...
        self._pw.setXRange(
            0, self._counting_logic.get_count_length() / self._counting_logic.get_count_frequency()
        )
        ...
        if self._counting_logic.module_state() == "locked":
            if 0 < self._counting_logic.countdata_smoothed[(self._display_trace - 1), -1] < 10:
                self._mw.count_value_Label.setText(
                    f"{self._counting_logic.countdata_smoothed[(self._display_trace - 1), -1]:,.6f}"
                )
            else:
                self._mw.count_value_Label.setText(
                    f"{self._counting_logic.countdata_smoothed[(self._display_trace - 1), -1]:,.0f}"
                )
    ...
    def restore_default_view(self):
        ...
        self._mw.addDockWidget(
            QtCore.Qt.DockWidgetArea(8), self._mw.slow_counter_parameters_DockWidget
        )
        self._mw.addDockWidget(
