...
class PIDGui(GUIBase):
    ...
    def on_activate(self):
        ...
        self.plot1.setLabel(
            "left",
            f"<font color={palette.c1.name()}>Process Value</font> and <font color={palette.c2.name()}>Setpoint</font>",
            units="unit",
        )
        self.plot1.setLabel("bottom", "Time", units="s")
        self.plot1.showAxis("right")
        self.plot1.getAxis("right").setLabel("Control Value", units="unit", color=palette.c3.name())
    ...
    def updateData(self):
        ...
        if self._pid_logic.get_enabled():
            self._mw.process_value_Label.setText(
                f"<font color={palette.c1.name()}>{self._pid_logic.history[0, -1]:,.3f}</font>"
            )
            self._mw.control_value_Label.setText(
                f"<font color={palette.c3.name()}>{self._pid_logic.history[1, -1]:,.3f}</font>"
            )
            self._mw.setpoint_value_Label.setText(
                f"<font color={palette.c2.name()}>{self._pid_logic.history[2, -1]:,.3f}</font>"
            )
