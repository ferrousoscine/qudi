...
class SwitchInterface(metaclass=InterfaceMetaclass):
    ...
    def _chk_refine_available_switches(switch_dict):
        ...
        assert all((isinstance(sw, str) and sw) for sw in switch_dict), (
            "Switch name must be non-empty string"
        )
        assert all(len(states) > 1 for states in switch_dict.values()), (
            "State tuple must contain at least 2 states"
        )
        assert all(
            all((s and isinstance(s, str)) for s in states) for states in switch_dict.values()
        ), "Switch states must be non-empty strings"
