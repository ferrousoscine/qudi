...
class SamplesWriteMethods:
    ...
    def _write_wfmx(
        ...
        for channel in analog_samples:
            ...
            with open(filepath, "ab") as wfmxfile:
                ...
                for start_ind in np.arange(
                    0, number_of_full_chunks * write_overhead_samples, write_overhead_samples
                ):
            ...
            with open(filepath, "ab") as tmpfile:
                if markers[0] not in digital_samples and markers[1] not in digital_samples:
                    ...
                    for start_ind in np.arange(
                        0, number_of_full_chunks * write_overhead_bytes, write_overhead_bytes
                    ):
    ...
    def _write_fpga(
        ...
        if len(digital_samples) != 8:
            self.log.warning(
                "FPGA pulse generator needs 8 digital channels. ({0} given)\n"
        ...
        if is_last_chunk and (total_number_of_samples % 32 != 0):
            ...
            self.log.warning(
                "FPGA pulse sequence length is no integer multiple of 32 samples. "
    ...
    def _write_pstream(
        ...
        if channel_number != 8:
            self.log.error(
                "Pulse streamer needs 8 digital channels. {0} is not allowed!".format(
                    channel_number
                )
            )
            return -1
        ...
        new_channel_indices = np.insert(
            new_channel_indices, new_channel_indices.size, [digital_samples.shape[0] - 1]
        )

        pulses = []
        for new_channel_index in range(1, new_channel_indices.size):
            pulse = [
                new_channel_indices[new_channel_index] - new_channel_indices[new_channel_index - 1],
                _convert_to_bitmask(
                    digital_samples[new_channel_indices[new_channel_index - 1] + 1, :]
                ),
            ]
            pulses.append(pulse)
        ...
        with open(filepath, "wb") as seq_file:
            ...
            for step_num, (ensemble_obj, seq_param) in enumerate(sequence_obj.ensemble_list):
                ...
                line_str += "{0:d}, {1:d}, {2:d}, {3:d}\r\n".format(
                    repeat, trigger_wait, go_to, event_jump_to
                )
    ...
    def _create_xml_file(self, number_of_samples, temp_dir=""):
        ...
        DataSetsCollection = ET.SubElement(
            root, "DataSetsCollection", xmlns="http://www.tektronix.com"
        )
        DataSets = ET.SubElement(
            DataSetsCollection, "DataSets", version="1", xmlns="http://www.tektronix.com"
        )
        ...
        CreatorProperties = ET.SubElement(
            ProductSpecific, "CreatorProperties", name="Basic Waveform"
        )
