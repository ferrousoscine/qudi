        ...
        for dirName, subDirList, fileList in os.walk(os.path.join(path, base)):
            for f in fileList:
                if fnmatch.fnmatch(f, "*.py"):
                    dirpath = dirName
                    dirpart = ""
                    while os.path.split(dirpath)[-1] not in [
                        "hardware",
                        "logic",
                        "gui",
                        "interface",
...
def check_qudi_modules(filelist):
    ...
    importerror = []
    importsuccess = []
    modules = {"hardware": {}, "logic": {}, "gui": {}, "interface": {}, "itask": {}, "pptask": {}}
    for f in filelist:
        try:
        ...
        for modpath, module in modules[base].items():
            for ifname, interface in modules["interface"].items():
                ...
                try:
                    ...
                    print(
                        "Interface {1} subclass check failed for {0}".format(
