                ...
                try:
                    if callable(getattr(mod, method)):
                        ...
                        if str(method).startswith("make_") and str(method).endswith("_fit"):
                            ...
                            if (
                                "twoD" in str(method)
                                and str(method).split("_")[1] not in twoD_fit_methods
                            ):
                            ...
                            if "twoD" in str(method):
                                try:  # if there is a given estimator it will be set or added
                                    if str(method).split("_")[1] in twoD_fit_methods:
                                        twoD_fit_methods[str(method).split("_")[1]] = (
                                            twoD_fit_methods[str(method).split("_")[1]].append(
                                ...
                                try:  # if there is a given estimator it will be set or added
                                    if (
                                        str(method).split("_")[1] in oneD_fit_methods
                                        and str(method).split("_")[2] is not None
                                    ):
                                        oneD_fit_methods[str(method).split("_")[1]].append(
                                            str(method).split("_")[2]
                                        )
                    ...
                    self.log.error(
                        "It was not possible to import element {} into FitLogic.".format(method)
                    )
        self.log.info(
            "Methods were included to FitLogic, but only if naming is right: "
...
def N15_testing():
    ...
    plt.legend(
        bbox_to_anchor=(0.0, 1.02, 1.0, 0.102), loc=3, ncol=2, mode="expand", borderaxespad=0.0
    )
    plt.show()
    ...
    if len(lorentz) >= 3:
        ...
        x_axis_min = x_axis[data_convolved.argmin()] - hf_splitting / 2.0
    else:
    ...
    params["l0_sigma"].set(value=sigma, min=minimal_sigma, max=maximal_sigma)
    params["l1_amplitude"].set(value=params["l0_amplitude"].value, max=-1e-6)
    params["l1_center"].set(
        value=params["l0_center"].value + hf_splitting, expr="l0_center+3.03*1e6"
    )
    params["l1_sigma"].set(
        value=params["l0_sigma"].value, min=minimal_sigma, max=maximal_sigma, expr="l0_sigma"
    )
...
def N15_testing2():
    ...
    p.add("l1_center", value=p["l0_center"].value + 3.03 * 1e6)
    p.add("l1_sigma", value=p["l0_sigma"].value)
    p.add("offset", value=100.0)

    data_nice = mod.eval(x=x_axis, params=p)

    data_noisy = data_nice + 14000 * np.random.normal(size=x_axis.shape)

    result = qudi_fitting.make_lorentziandouble_fit(
        x_axis, data_noisy, estimator=qudi_fitting.estimate_lorentziandouble_N15
    )
...
def N14_testing():
    ...
    params["l0_amplitude"].set(value=amplitude, max=-1e-6)
    params["l0_center"].set(value=x_axis_min)
    params["l0_sigma"].set(value=sigma, min=minimal_linewidth, max=maximal_linewidth)
    params["l1_amplitude"].set(value=amplitude, max=-1e-6)
    params["l1_center"].set(value=x_axis_min + 2.15 * 1e6, expr="l0_center+2.15*1e6")
    params["l1_sigma"].set(
        value=sigma, min=minimal_linewidth, max=maximal_linewidth, expr="l0_sigma"
    )
    params["l2_amplitude"].set(value=amplitude, max=-1e-6)
    params["l2_center"].set(value=x_axis_min + 2.15 * 1e6, expr="l0_center+4.3*1e6")
    params["l2_sigma"].set(
        value=sigma, min=minimal_linewidth, max=maximal_linewidth, expr="l0_sigma"
    )
    params["offset"].set(value=offset)
...
def N14_testing2():
    ...
    result = qudi_fitting.make_lorentziantriple_fit(
        x_axis, data_noisy, estimator=qudi_fitting.estimate_lorentziantriple_N14
    )
...
def N14_testing_data():
    ...
    path = os.path.abspath(
        r"C:\Users\astark\Dropbox\Doctorwork\2016\2016-07\2016-07-05 N14 fit fails\20160705-1147-41_n14_fit_fails_60MHz_span_ODMR_data.dat"
    )
    ...
    parameters.add(
        "l1_center",
        value=parameters["l0_center"].value + 2.15 * 1e6,
        expr="lorentz0_center+2.15*1e6",
    )
    parameters.add(
        "l1_sigma",
        value=parameters["l0_sigma"].value,
        min=minimal_linewidth,
    ...
    parameters.add(
        "l2_center",
        value=parameters["l1_center"].value + 2.15 * 1e6,
        expr="lorentz0_center+4.3*1e6",
    )
    parameters.add(
        "l2_sigma",
        value=parameters["l0_sigma"].value,
...
def N14_testing_data2():
    ...
    # that this is a raw input. The os package will do the rest.
    path = os.path.abspath(
        r"C:\Users\astark\Dropbox\Doctorwork\Software\QuDi-Git\qudi\pulsedODMRdata.csv"
    )
...
def gaussianpeak_testing():
    ...
    data_noisy = mod_final.eval(
        x=x_axis, amplitude=ampl, center=center, sigma=sigma, offset=offset
...
def gaussianlinearoffset_testing():
    ...
    data_noisy = mod_final.eval(
        x=x_axis, slope=slope, amplitude=amplitude, center=center, sigma=sigma, offset=offset
    ) + 12000 * abs(np.random.normal(size=x_axis.shape))

    # try at first a fit with the ordinary gauss function
    res_ordinary_gauss = qudi_fitting.make_gaussoffsetpeak_fit(x_axis=x_axis, data=data_noisy)
    ...
    res_linear = qudi_fitting.make_linear_fit(x_axis=x_axis, data=data_subtracted)
...
def two_gaussian_peak_testing():
    ...
    error, params_lor = qudi_fitting.estimate_lorentziandouble_dip(
        x_axis=x_axis,
        data=-data_noisy,
        params=params_lor,
...
def two_gaussian_peak_testing2():
    ...
    result = qudi_fitting.make_twogausspeakoffset_fit(x_axis=x_axis, data=data_noisy)
...
def two_gaussian_dip_testing2():
    ...
    result = qudi_fitting.make_twogaussdipoffset_fit(x_axis=x_axis, data=data_noisy)
...
def gaussian_twoD_testing():
    ...
    ax.hold(True)
    ax.imshow(
        result.data.reshape(len(y), len(x)),
        cmap=plt.cm.jet,
        origin="bottom",
        extent=(x.min(), x.max(), y.min(), y.max()),
        interpolation="nearest",
    )
    ax.contour(x, y, result.best_fit.reshape(len(y), len(x)), 8, colors="w")
    plt.show()
    #    plt.close('all')
...
def useful_object_variables():
    ...
    data_noisy = model.eval(x=x, params=p) + 0.5 * np.random.normal(size=x.shape)
    para = Parameters()
...
def double_lorentzdip_testing():
    ...
    p.add("l0_sigma", value=abs(np.random.random(1) * 1.0 + 1.0))
    p.add("l1_center", value=np.random.random(1) * 150.0 + 2800)
    p.add("l1_sigma", value=abs(np.random.random(1) * 4.0 + 1.0))
    ...
    result = qudi_fitting.make_lorentziandouble_fit(
        x_axis=x_axis, data=data_noisy, estimator=qudi_fitting.estimate_lorentziandouble_dip
    )
    ...
    ret_val = qudi_fitting._search_double_dip(
    ...
    if dip0_arg == dip1_arg:
        lorentz0_amplitude = data_level[dip0_arg] / 2.0
    ...
    smoothing_spline = 1  # must be 1<= smoothing_spline <= 5
    fit_function = InterpolatedUnivariateSpline(x_axis, data_level, k=smoothing_spline)
    numerical_integral_0 = fit_function.integral(x_axis[sigma0_argleft], x_axis[sigma0_argright])
    ...
    if lorentz0_center < lorentz1_center:
        ...
        params["l0_center"].set(
            value=lorentz0_center,
            min=(x_axis[0]) - n_steps * stepsize,
            max=(x_axis[-1]) + n_steps * stepsize,
        )
        params["l1_amplitude"].set(value=lorentz1_amplitude, max=-0.01)
        params["l1_sigma"].set(value=lorentz1_sigma, min=stepsize / 2, max=full_width * 4)
        params["l1_center"].set(
            value=lorentz1_center,
            min=(x_axis[0]) - n_steps * stepsize,
            max=(x_axis[-1]) + n_steps * stepsize,
        )
    else:
        params["l0_amplitude"].set(value=lorentz1_amplitude, max=-0.01)
        params["l0_sigma"].set(value=lorentz1_sigma, min=stepsize / 2, max=full_width * 4)
        params["l0_center"].set(
            value=lorentz1_center,
            min=(x_axis[0]) - n_steps * stepsize,
            max=(x_axis[-1]) + n_steps * stepsize,
        ...
        params["l1_sigma"].set(value=lorentz0_sigma, min=stepsize / 2, max=full_width * 4)
        params["l1_center"].set(
            value=lorentz0_center,
            min=(x_axis[0]) - n_steps * stepsize,
            max=(x_axis[-1]) + n_steps * stepsize,
...
def double_lorentzdip_testing2():
    ...
    result = qudi_fitting.make_lorentziandouble_fit(
        x_axis=x, data=data_noisy, estimator=qudi_fitting.estimate_lorentziandouble_dip
    )
...
def double_lorentzpeak_testing2():
    ...
    result = qudi_fitting.make_lorentziandouble_fit(
        x_axis=x, data=data_noisy, estimator=qudi_fitting.estimate_lorentziandouble_peak
    )
...
def double_lorentzian_fixedsplitting_testing():
    ...
    for ii in range(1):
        ...
        p.add("lorentz0_sigma", value=abs(np.random.random(1) * 2.0 + 1.0))
        p.add("lorentz1_center", value=p["lorentz0_center"] + 20)
        p.add("lorentz1_sigma", value=abs(np.random.random(1) * 2.0 + 1.0))
        ...
        result = qudi_fitting.make_doublelorentzian_fit(
            axis=x, data=data_noisy, add_parameters=para
        )
        ...
        plt.plot((x[sigma0_argleft], x[sigma0_argleft]), (data_noisy.min(), data_noisy.max()), "b-")
        plt.plot(
            (x[sigma0_argright], x[sigma0_argright]), (data_noisy.min(), data_noisy.max()), "b-"
        )

        plt.plot((x[sigma1_argleft], x[sigma1_argleft]), (data_noisy.min(), data_noisy.max()), "k-")
        plt.plot(
            (x[sigma1_argright], x[sigma1_argright]), (data_noisy.min(), data_noisy.max()), "k-"
        )
...
def lorentziandip_testing():
    ...
    params.add("amplitude", value=-20.0)
    params.add("center", value=920.0)
    ...
    if data_max > abs(data_min):
        qudi_fitting.log.warning(
            "The lorentzian estimator set the peak to the minimal value, if "
    ...
    params["center"].set(
        value=x_zero, min=(x_axis[0]) - n_steps * stepsize, max=(x_axis[-1]) + n_steps * stepsize
    )
    params["offset"].set(value=offset)
    #    para=Parameters()
...
def lorentziandip_testing2():
    ...
    result = qudi_fitting.make_lorentzian_fit(
        x_axis=x_axis,
        data=data_noisy,
        units=["MHz"],
...
def lorentzianpeak_testing2():
    ...
    result = qudi_fitting.make_lorentzian_fit(
        x_axis=x_axis, data=data_noisy, estimator=qudi_fitting.estimate_lorentzian_peak
    )
...
def powerfluorescence_testing():
    ...
    print(
        ...
        mod.param_names,
        " with the independet variable",
        mod.independent_vars,
    )

    params["I_saturation"].value = 200.0
    params["slope"].value = 0.25
    params["intercept"].value = 2.0
    params["P_saturation"].value = 100.0
    data_noisy = mod.eval(x=x, params=params) + 10 * np.random.normal(size=x.shape)

    para = dict()
    para["I_saturation"] = {"value": 152.0}
    para["slope"] = {"value": 0.3, "vary": True}
    para["intercept"] = {"value": 0.3, "vary": False, "min": 0.0}
    para["P_saturation"] = {"value": 130.0}
...
def double_gaussian_odmr_testing():
    ...
    for ii in range(1):
        ...
        (
            ...
            dip1_arg,
            sigma1_argright,
        ) = qudi_fitting._search_double_dip(
            x,
            data_level,
            threshold_fraction=threshold_fraction,
            minimal_threshold=minimal_threshold,
        ...
        result = qudi_fitting.make_doublegaussian_fit(
            x,
            data_noisy,
            estimator="odmr_dip",
            threshold_fraction=threshold_fraction,
...
def sine_testing():
    ...
    for tries in range(len(diff_array)):
        if np.isclose(min_x_diff, 0.0):
            ...
            if len(diff_array) == 0:
                qudi_fitting.log.error(
                    "The passed x_axis for the sinus estimation contains the same values! Cannot do the fit!"
                )
...
def sine_testing2():
    ...
    update_dict = {"phase": {"vary": False, "value": np.pi / 2.0}}

    result = qudi_fitting.make_sine_fit(x_axis=x_axis, data=data_noisy, add_params=update_dict)
...
def sine_testing_data():
    ...
    path = os.path.abspath(
        r"C:\Users\astark\Dropbox\Doctorwork\2016\2016-10\2016-10-24_06_sensi_error_scaling_30min"
    )
...
def twoD_gaussian_magnet():
    ...
    try:
        ...
        xx, yy = np.meshgrid(x, y)
        axes = (xx.flatten(), yy.flatten())
        theta_here = np.random.normal(0, 100) / 360.0 * (2 * np.pi)
        gmod, params = qudi_fitting.make_twoDgaussian_model()
        data = gmod.eval(
            x=axes,
            amplitude=amplitude,
    ...
    print(result.fit_report())
    print(
        "Maximum after fit (GHz): ",
        result.params["offset"].value + result.params["amplitude"].value,
    )
    #            FIXME: What does "Tolerance seems to be too small." mean in message?
    #            print(result.message)
    plt.close("all")
...
def poissonian_testing():
    ...
    p = Parameters()
    p.add("mu", value=mu)
    p.add("amplitude", value=200.0)

    data_noisy = mod.eval(x=x, params=p) * np.array(
        (1 + 0.001 * np.random.normal(size=x.shape) * p["amplitude"].value)
    )
...
def double_poissonian_testing():
    ...
    data_noisy = np.array(mod.eval(x=x, params=parameter)) * np.array(
        (1 + 0.2 * np.random.normal(size=x.shape)) * parameter["p1_amplitude"].value
    )
...
def double_poissonian_testing_data():
    ...
    # calculation!
    path = os.path.abspath(
        r"C:\Users\astark\Desktop\test_poisson\150902_21h58m_Rabi_171p0_micro-s_Trace.asc"
    )
    ...
    search_res = qudi_fitting._search_double_dip(
        x_axis_interpol,
        data_smooth * (-1),
        threshold_fraction,
        minimal_threshold,
    ...
    params["p0_amplitude"].value = interpol_hist[dip0_arg] / qudi_fitting.poisson(
        x_axis_interpol[dip0_arg], x_axis_interpol[dip0_arg]
    )

    params["p1_mu"].value = x_axis_interpol[dip1_arg]
    params["p1_amplitude"].value = interpol_hist[dip1_arg] / qudi_fitting.poisson(
...
def sineexponentialdecay_testing():
    ...
    plt.plot(freq[: int(len(freq) / 2)], fourier_power[: int(len(freq) / 2)], "-or")
    plt.xlim(0, 0.5)
    plt.show()

    result = qudi_fitting.make_sineexponentialdecay_fit(
        x_axis=x_axis, data=data_noisy, add_parameters=None
    )
...
def sineexponentialdecay_testing_data():
    ...
    path = os.path.abspath(
        r"C:\Users\astark\Dropbox\Doctorwork\2016\2016-11\2016-11-06_04_ddrive_signal_-35p19_-61p15dBm_ana"
    )
    ...
    params["offset"].set(value=offset)
    params["lifetime"].set(
        min=2 * (x_axis[1] - x_axis[0]), max=1 / (abs(dft_x[1] - dft_x[0]) * 0.5)
    )
...
def sineexponentialdecay_testing_data2():
    ...
    path = os.path.abspath(
        r"C:\Users\astark\Dropbox\Doctorwork\2016\2016-11\2016-11-04_02_sdrive_signal_-49p15_-61p15dBm_ana"
    )
...
def stretchedexponentialdecay_testing():
    ...
    result = qudi_fitting.make_stretchedexponentialdecay_fit(
        axis=x_axis, data=data_noisy, add_parameters=None
    )
    ...
    try:
        ...
        X = np.log(x_axis[a:b])
        linear_result = qudi_fitting.make_linear_fit(
            axis=X, data=double_lg_data, add_parameters=None
        )
...
def stretched_sine_exponential_decay_testing_data():
    ...
    result = qudi_fitting.make_sinestretchedexponentialdecay_fit(x_axis=x_axis, data=data)
...
def fit_data():
    ...
    result = qudi_fitting.make_lorentzian_fit(
        axis=data[:, 0], data=data[:, 3], add_parameters=params
    )
    print(result.fit_report())
    plt.plot(data[:, 0], -data[:, 3] + 2, "b-o", label="data mean")
    #    plt.plot(data[:,0],data[:,1],label="data")
    #    plt.plot(data[:,0],data[:,2],label="data")
    plt.plot(data[:, 0], -result.best_fit + 2, "r-", linewidth=2.0, label="fit")
    ...
    plt.show()
    savedata = [
        [data[ii, 0], -data[ii, 3] + 2, -result.best_fit[ii] + 2] for ii in range(len(data[:, 0]))
    ]
...
def double_exponential_testing2():
    ...
    res = qudi_fitting.make_doubleexponentialdecayoffset_fit(x_axis=x_axis, data=noisy_data)
...
def stretched_exponential_decay_testing():
    ...
    res = qudi_fitting.make_stretchedexponentialdecayoffset_fit(x_axis=x_axis, data=noisy_data)
...
def two_sine_two_exp_decay_offset_testing():
    ...
    params["e1_lifetime"].set(
        value=result1.params["lifetime"].value, min=2 * (x_axis[1] - x_axis[0])
    )
    ...
    params["e2_lifetime"].set(
        value=result2.params["lifetime"].value, min=2 * (x_axis[1] - x_axis[0])
    )
...
def two_sine_two_exp_decay_offset_testing2():
    ...
    result = qudi_fitting.make_sinedoublewithtwoexpdecay_fit(
        x_axis=x_axis, data=noisy_data, estimator=qudi_fitting.estimate_sinedoublewithtwoexpdecay
    )
...
def three_sine_three_exp_decay_offset_testing():
    ...
    params["e1_lifetime"].set(value=res1.params["lifetime"].value, min=2 * (x_axis[1] - x_axis[0]))
    ...
    params["e2_lifetime"].set(value=res2.params["lifetime"].value, min=2 * (x_axis[1] - x_axis[0]))
    ...
    params["e3_lifetime"].set(value=res3.params["lifetime"].value, min=2 * (x_axis[1] - x_axis[0]))
...
def voigt_testing():
    ...
    params.add("amplitude", value=30.0)
    params.add("center", value=920.0)
    ...
    if x_axis[0] - x_axis[1] > 0:
        ...
        params["center"].set(value=amplitude, vary=True, min=x_axis[-1], max=x_axis[0])
