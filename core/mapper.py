...
from qtpy.QtCore import QCoreApplication, QThread, QTimer
...
class Mapper:
    ...
    def _get_property_from_widget(self, widget):
        ...
        if isinstance(widget, QAbstractButton):
            ...
            raise Exception(f"Property of widget {repr(widget)} could not be determined.")
    ...
    def add_mapping(
        ...
        if isinstance(model_getter, str):
            ...
            if attr is None:
                raise Exception(f"Model has no attribute {model_getter}")
            if isinstance(attr, property):
                ...
                if model_setter is None:
                    ...
                    if model_getter is None:
                        raise Exception(f"Attribute {model_property_name} of model is readonly.")
            else:
                ...
                if not callable(model_getter):
                    raise Exception(f"Attribute {model_getter_name} of model is not callable.")
        if isinstance(model_setter, str):
            ...
            if not callable(model_setter):
                raise Exception(f"Attribute {model_setter_name} of model is not callable")
    ...
    def remove_mapping(self, widget, widget_property_name=""):
        ...
        if key not in self._mappings:
            raise Exception(f"Widget {repr(widget)} is not mapped.")
        # disconnect signals
    ...
    def submit_policy(self, policy):
        ...
        if policy not in [SUBMIT_POLICY_AUTO, SUBMIT_POLICY_MANUAL]:
            raise Exception(f"Unknown submit policy {policy}")
