...
# Modern versions of pyzmq (17+) work with default tornado and asyncio event loops
# No need to install zmq.eventloop explicitly anymore
try:
    import zmq

    logger.info(f"ZMQ version: {zmq.zmq_version()}")
except ImportError:
    logger.error("ZMQ not available, probably no IPython possible!")
        ...
        if sys.platform == "win32":
            if self.interrupt or self.parent_handle:
                self.poller = ParentPollerWindows(
                    lambda: self.quitProxy(manager), self.interrupt, self.parent_handle
                )
                self.poller.start()
