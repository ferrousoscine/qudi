...
class Qudi:
    ...
    def startKernel(self, connfile):
        ...
        logging.info(f"Kernel up: {self.kernelid}")

    def stopKernel(self):
        logging.info(f"Shutting down: {self.kernelid}")
        sys.stdout.flush()
        m = self.getModule("kernellogic")
    ...
    def getConfigFile(self):
        ...
        if os.path.isfile(loadConfigFile):
            logging.info(f"load.cfg config file found at {loadConfigFile}")
            try:
                ...
                if "configfile" in confDict and isinstance(confDict["configfile"], str):
                    ...
                    if os.path.isfile(configFile):
                        logging.info(f"Config file found at {configFile}")
                        return configFile
    ...
    def getConfigFromFile(self, configfile):
        ...
        if "module_server" in cfg["global"]:
            ...
            logging.warning(
                "Deprecated remote server settings. Please update to new style. See documentation."
            )
...
def install_kernel():
    ...
    try:
        ...
        shutil.copy(
            os.path.join(logopath, "logo-qudi-32x32.png"),
        ...
        logging.info(f"Installed kernelspec qudi in {dest}")
    except OSError as e:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print(
            f"qudikernel usage is {sys.argv[0]} <connectionfile> or {sys.argv[0]} install",
            file=sys.stderr,
        )
