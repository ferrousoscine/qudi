...
class RemoteObjectManager(QObject):
            ...
            def exposed_getModule(self, name):
                ...
                if name in self.modules.storage:
                    ...
                    for base in ["hardware", "logic", "gui"]:
                        logger.info(f"remotesearch: {name}")
                        if (
                            name in self._manager.tree["defined"][base]
                            and "remoteaccess" in self._manager.tree["defined"][base][name]
                        ):
                            self._manager.startModule(base, name)
                            logger.info(f"remoteload: {base}{name}")
                    if name in self.modules.storage:
                        return self.modules.storage[name]
                    else:
    ...
    def createServer(self, hostname, port, certfile=None, keyfile=None, cacertfile=None):
        ...
        thread.start()
        logger.info(f"Started module server at {hostname} on port {port}")
    ...
    def shareModule(self, name, obj):
        ...
        if name in self.sharedModules.storage:
            logger.warning(f"Module {name} already shared.")
        self.sharedModules.add(name, obj)
        logger.info(f"Shared module {name}.")
    ...
    def unshareModule(self, name):
        ...
        """
        if name in self.sharedModules.storage:
            logger.error(f"Module {name} was not shared.")
    ...
    def getRemoteModuleUrl(self, url, certfile=None, keyfile=None, cacertsfile=None):
        ...
        return self.getRemoteModule(
            parsed.hostname, parsed.port, name, certfile, keyfile, cacertsfile
        )
    ...
    def getRemoteModule(self, host, port, name, certfile=None, keyfile=None, cacertsfile=None):
        ...
        """
        module = RemoteModule(
            host, port, name, certfile=certfile, keyfile=keyfile, cacertsfile=cacertsfile
        )
    ...
    def run(self):
        ...
        if self.certfile is not None and self.keyfile is not None:
            if not os.path.exists(self.certfile):
                raise Exception(f"SSL certificate {self.certfile} does not exist.")
            if not os.path.exists(self.keyfile):
                raise Exception(f"SSL private key file {self.keyfile} does not exist.")
            if (self.cacertsfile is not None) and (not os.path.exists(self.cacertsfile)):
                logger.warning(f"SSL CA certificates file {self.cacertsfile} does not exist.")
            authenticator = SSLAuthenticator(
                server_key_file=self.keyfile,
    ...
    def __init__(self, host, port, name, certfile=None, keyfile=None, cacertsfile=None):
        if certfile is not None and keyfile is not None:
            if not os.path.exists(certfile):
                raise Exception(f"SSL certificate {certfile} does not exist.")
            if not os.path.exists(keyfile):
                raise Exception(f"SSL private key file {keyfile} does not exist.")
            if (cacertsfile is not None) and (not os.path.exists(cacertsfile)):
                logger.warning(f"SSL CA certificates file {cacertsfile} does not exist.")
